...
4. **`.cs` 文件里不写解释性注释。** 理由、决策依据写进 `Documentation~/` 与计划正文。代码要保持干净。
...
WorldObject 收到输入 / 回调
  ...
  → WorldObject 订阅 event（或读 Data）刷新表现
```
...
| `WorldObject` | **只读**，永不直接调 Data 的修改方法 |
| `Service` | 跨领域场景可写，同样走 Data 的修改方法 |

### 2.4 为什么不让 WorldObject 直接写

1. **中间流程** —— 改一个数值往往牵动一串副作用。真实例子：扣血 → 血量归 0 → 死亡判定 → 掉落 / 播放死亡动画 / 发事件 / 存档脏标记。WO 直接调 `data.ApplyDamage(10)`，数值变了但这串流程没人走，世界不会反应。
2. **不变量** —— Data 自己守约束（血量不为负、数量不超上限）。绕过 Data 的方法直接写字段，不变量就破了。
3. **单一写路径** —— 两个写入口时，出 bug 无法判断是谁改的；存档脏标记、撤销、回放也都依赖单一写入口。

WorldObject 只读不会被卡住：表现刷新由 Controller 的 `event` 驱动，不靠 WO 自己写状态。

### 2.5 框架能强制到什么程度

Data 的修改方法必然是 `public`，WO 拿到 Data 实例就能调，**类型上无法禁止**。框架做到的是三件事：

1. `WorldObject` 基类不提供任何 Data 写入口（只有只读属性 `Data`）
2. `WorldObject` 只持有 `Controller` —— 要做事只能调 Controller 的意图方法（单向依赖正好支撑这条）
3. 本规则写进 `编码规范.md` 与 `AI约束.md`，并作为评审检查点

**代价（认下）**：WO 直接调 `data.ApplyDamage(...)` 编译器拦不住，绕过流程的 bug 只能靠评审发现。若将来要真正强制，唯一手段是把修改方法移到 Controller，但那样 Data 退化成纯数据袋、不变量没人守 —— 不采用。
